...
const filter = (rule, params) => {
    if (rule) {
        const keysParam1 = Object.keys(params);
        const keysParam2 = Object.keys(rule);
        keysParam1.forEach(v => {
            if (!keysParam2.includes(v)) {
                delete params[v];
            }
        });
    }
...
module.exports = { setKey, getKey, generateFailure, checkRouter, sendMsg, generateRandomString, filter };
